    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const client = createOpencodeClient({
          ...
          headers: Flag.PREMISE_SERVER_PASSWORD
            ? {
                Authorization: `Basic ${Buffer.from(`${Flag.PREMISE_SERVER_USERNAME ?? "opencode"}:${Flag.PREMISE_SERVER_PASSWORD}`).toString("base64")}`,
              }
            : undefined,
        ...
        const plugins = Flag.PREMISE_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.PREMISE_PURE && cfg.plugin_origins?.length) {
