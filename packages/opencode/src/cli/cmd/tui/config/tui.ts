...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const projectFiles = Flag.PREMISE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  if (Flag.PREMISE_TUI_CONFIG) {
    const configFile = Flag.PREMISE_TUI_CONFIG
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.PREMISE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.PREMISE_CONFIG_DIR) continue
