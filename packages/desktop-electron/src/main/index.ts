...
const APP_IDS: Record<string, string> = {
  dev: "ai.premise.desktop.dev",
  beta: "ai.premise.desktop.beta",
  prod: "ai.premise.desktop",
}
const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.premise.desktop.dev"
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Premise Dev")
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("premise://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    app.setAsDefaultProtocolClient("premise")
    registerRendererProtocol()
    setDockIcon()
    setupAutoUpdater()
    await initialize()
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "premise", "premise.db"))
}
